...
class Login_Window:
    def __init__(self, root):
        ...
        btn_reg = Button(login_frame, text="Register New Account?", command=self.register_window, font=("times new roman", 14), bg="white", bd=0, fg="#B00857", cursor="hand2")
        btn_reg.place(x=250, y=320)

        btn_forget = Button(login_frame, text="Forget Password?", command=self.forget_password, font=("times new roman", 14), bg="white", bd=0, fg="#B00857", cursor="hand2")
        ...
        self.lbl = Label(self.root, text="\nDev Kumar", font=("Book Antiqua", 25, "bold"), fg="white", compound=BOTTOM, bg="#081923", bd=0)
    ...
    def forget_password(self):
        ...
        if self.txt_email.get() == "":
            messagebox.showerror("Error", "Please Enter Email Address First", parent=self.root)
            return
        ...
        try:
            ...
            if row is None:
                ...
                title = Label(self.root2, text="Reset Password", font=("times new roman", 20, "bold"), bg="white", fg="#3f51b5")
                title.pack(pady=20)

                lbl_new = Label(self.root2, text="New Password", font=("times new roman", 15, "bold"), bg="white")
                lbl_new.place(x=50, y=100)

                self.txt_new_pass = Entry(self.root2, font=("times new roman", 15), bg="lightyellow", show="*")
                self.txt_new_pass.place(x=50, y=140, width=300)

                lbl_confirm = Label(self.root2, text="Confirm Password", font=("times new roman", 15, "bold"), bg="white")
                lbl_confirm.place(x=50, y=190)

                self.txt_confirm_pass = Entry(self.root2, font=("times new roman", 15), bg="lightyellow", show="*")
                self.txt_confirm_pass.place(x=50, y=230, width=300)

                btn_reset = Button(self.root2, text="Reset Password", command=self.reset_password, font=("times new roman", 15, "bold"), bg="#3f51b5", fg="white", cursor="hand2")
    ...
    def reset_password(self):
        ...
        if self.txt_new_pass.get() == "" or self.txt_confirm_pass.get() == "":
            ...
            messagebox.showerror("Error", "Passwords Do Not Match", parent=self.root2)
            ...
            try:
                ...
                cur.execute("UPDATE teacher SET password=%s WHERE email=%s", (self.txt_new_pass.get(), self.txt_email.get()))
                con.commit()
    ...
    def login(self):
        ...
        if self.txt_email.get() == "" or self.txt_pass_.get() == "":
            ...
            try:
                ...
                cur.execute("SELECT * FROM teacher WHERE email=%s AND password=%s", (self.txt_email.get(), self.txt_pass_.get()))
                row = cur.fetchone()
