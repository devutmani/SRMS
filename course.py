...
class CourseClass:
    def __init__(self, root):
        ...
        Button(self.root, text="Search", command=self.search, font=("goudy old style", 15, "bold"), bg="#2196f3", fg="white", cursor="hand2").place(x=1070, y=60, width=120, height=28)
    ...
    def add(self):
        try:
            ...
            cur.execute("INSERT INTO course(name, duration, charges, description) VALUES(%s,%s,%s,%s)", (self.var_course.get(), self.var_duration.get(), self.var_charges.get(), self.txt_description.get("1.0", END)))
    ...
    def update(self):
        try:
            ...
            cur.execute("UPDATE course SET duration=%s, charges=%s, description=%s WHERE name=%s", (self.var_duration.get(), self.var_charges.get(), self.txt_description.get("1.0", END), self.var_course.get()))
